...
class InMemoryRepo:
    ...
    async def create(self, t): self.tasks[t.task_id] = t
    async def get(self, tid): return self.tasks.get(tid)
    async def update(self, t): self.tasks[t.task_id] = t
    async def mark_stale_as_interrupted(self): return 0


class NoopWorker:
    def __init__(self): self.jobs = []
    async def enqueue(self, job): self.jobs.append(job)
...
def test_transcript_failed_on_transcribe_409(client, repo):
    repo.tasks["t"] = Task(task_id="t", source_kind="audio", status=TaskStatus.FAILED,
...
def test_result_returns_zip(client, repo, tmp_path):
    ...
    repo.tasks["t"] = Task(task_id="t", source_kind="audio",
                           status=TaskStatus.DONE, result_path=str(zip_path))
    r = client.get("/api/v1/tasks/t/result")
    assert r.status_code == 200
